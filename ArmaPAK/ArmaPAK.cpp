...
extern "C" __declspec(dllexport) int DCPCALL PackFiles(
	...
	LogError("[PackFiles] Packing (writing) to PAK archives is not supported by this plugin.");
	return E_NOT_SUPPORTED;
}

extern "C" __declspec(dllexport) int DCPCALL PackFilesW(
	WCHAR* PackedFile,
	WCHAR* Files,
	int Flags,
	tChangeVolProc ChangeVolProc,
	...
	LogError("[PackFilesW] Packing (writing) to PAK archives is not supported by this plugin.");
	return E_NOT_SUPPORTED;
}

extern "C" __declspec(dllexport) int DCPCALL PackToMem(
	char* PackedFile,
	char* Files,
	int Flags,
	tProcessDataProc ProcessDataProc,
	...
	try {
		if (!currentArchive) {
			LogError("[PackToMem] Archive not open.");
			return E_BAD_ARCHIVE;
		}

		std::string filenameToFind = Files;
		...
		if (!entry) {
			LogError("[PackToMem] File not found in archive: " + filenameToFind);
			return E_NO_FILES;
		}

		if (entry->isDirectory) {
			LogInfo("[PackToMem] Skipping directory entry: " + entry->name);
			return 0;
		}

		LogInfo("[PackToMem] Starting memory extraction for: " + entry->name +
			", originalSize=" + std::to_string(entry->originalSize));
		...
		if (g_EnableEddsConversion) {
			...
			if (ext == ".edds") {
				LogInfo("[PackToMem] EDDS conversion to DDS requested for memory extraction (in-memory).");
				...
				if (ddsContent.has_value()) {
					...
					LogInfo("[PackToMem] EDDS converted to DDS successfully in memory. New size: " + std::to_string(processedContent.size()));
				}
				else {
					LogError("[PackToMem] EDDS conversion failed for " + entry->name + ". Streaming original (unconverted) content.");
				}
			}
		}
		...
		while (pos < processedContent.size()) {
			...
			if (result == 0) {
				LogError("[PackToMem] Streaming aborted by user.");
				return E_EABORTED;
			}
		...
		if (result == 0) {
			LogError("[PackToMem] Final streaming aborted by user.");
			return E_EABORTED;
		}

		LogInfo("[PackToMem] Memory extraction finished successfully.");
		return 0;
	}
	catch (const std::exception& ex) {
		LogError("[PackToMem] EXCEPTION: " + std::string(ex.what()));
		if (ProcessDataProc) {
