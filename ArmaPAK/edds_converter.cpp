...
std::optional<std::vector<uint8_t>> ConvertToDDSInMemory(
	...
	if (eddsContent.size() < 128) {
		LogError("[ConvertToDDSInMemory] Invalid EDDS content size for: " + entryName);
		return std::nullopt;
	}
	...
	std::vector<uint8_t> ddsHeader(128);
	std::memcpy(ddsHeader.data(), eddsContent.data() + readPos, 128);
	readPos += 128;

	if (std::memcmp(ddsHeader.data(), "DDS ", 4) != 0) {
		LogError("[ConvertToDDSInMemory] DDS magic number not found. Invalid EDDS file: " + entryName);
		return std::nullopt;
	}
	...
	if (ddsHeader[84] == 'D' && ddsHeader[85] == 'X' &&
		ddsHeader[86] == '1' && ddsHeader[87] == '0') {
		if (eddsContent.size() < readPos + 20) {
			LogError("[ConvertToDDSInMemory] Incomplete DX10 header in file: " + entryName);
			return std::nullopt;
		}
		ddsHeaderDx10.resize(20);
		std::memcpy(ddsHeaderDx10.data(), eddsContent.data() + readPos, 20);
		readPos += 20;
	}
	...
	while (readPos + 8 <= eddsContent.size()) {
		char blockName[4] = { 0 };
		int32_t size = 0;

		std::memcpy(blockName, eddsContent.data() + readPos, 4);
		readPos += 4;

		std::memcpy(&size, eddsContent.data() + readPos, 4);
		readPos += 4;
		...
		if (size < 0 || static_cast<uint32_t>(size) > 100 * 1024 * 1024) {
			size = static_cast<int32_t>(SwapEndian(static_cast<uint32_t>(original_size)));
			if (size < 0 || static_cast<uint32_t>(size) > 100 * 1024 * 1024) {
				LogError("[ConvertToDDSInMemory] Invalid block size even after byte order swap for block type '" + blockStr + "' in " + entryName + ". Aborting block search.");
				return std::nullopt;
			}
		}
	...
	if (blocks.empty()) {
		LogError("[ConvertToDDSInMemory] No data block (COPY/LZ4) found in " + entryName + ".");
		return std::nullopt;
	}
	...
	for (size_t i = 0; i < blocks.size(); i++) {
		...
		if (block.size <= 0) {
			LogError("[ConvertToDDSInMemory] [WARN] Invalid block size " + std::to_string(block.size) + " at block #" + std::to_string(i) + " (type: " + block.type + ") in " + entryName + ". Skipping.");
			continue;
		}

		if (readPos + static_cast<size_t>(block.size) > eddsContent.size()) {
			LogError("[ConvertToDDSInMemory] [ERROR] Block content read out of bounds at block #" + std::to_string(i) + " in " + entryName + ". Aborting block processing.");
			return std::nullopt;
		}

		if (block.type == "COPY") {
			currentBlockData.resize(static_cast<size_t>(block.size));
			std::memcpy(currentBlockData.data(), eddsContent.data() + readPos, block.size);
			readPos += block.size;
		}
		else if (block.type == "LZ4 ") {
			if (readPos + sizeof(uint32_t) > eddsContent.size()) {
				LogError("[ConvertToDDSInMemory] [ERROR] Cannot read decompressed size for LZ4 block #" + std::to_string(i) + " in " + entryName + ". Aborting block processing.");
				return std::nullopt;
			}
			uint32_t decompressedTotalSize = 0;
			std::memcpy(&decompressedTotalSize, eddsContent.data() + readPos, sizeof(decompressedTotalSize));
			readPos += sizeof(decompressedTotalSize);

			size_t compressedBlockActualSize = static_cast<size_t>(block.size) - sizeof(decompressedTotalSize);
			if (static_cast<int>(compressedBlockActualSize) <= 0) {
				LogError("[ConvertToDDSInMemory] [WARN] LZ4 block size is too small at #" + std::to_string(i) + " in " + entryName + ". Skipping.");
				continue;
			}
			if (readPos + compressedBlockActualSize > eddsContent.size()) {
				LogError("[ConvertToDDSInMemory] [ERROR] LZ4 compressed data read out of bounds at block #" + std::to_string(i) + " in " + entryName + ". Aborting block processing.");
				return std::nullopt;
			}

			const char* compressedData = reinterpret_cast<const char*>(eddsContent.data() + readPos);
			readPos += compressedBlockActualSize;
			...
			if (!lz4Stream) {
				LogError("[ConvertToDDSInMemory] [ERROR] Failed to create LZ4 decode stream for block #" + std::to_string(i) + " in " + entryName + ". Aborting block processing.");
				return std::nullopt;
			}

			size_t outPos = 0;
			size_t readChunkPos = 0;

			while (readChunkPos < compressedBlockActualSize) {
				if (readChunkPos + sizeof(int32_t) > compressedBlockActualSize) {
					LogError("[ConvertToDDSInMemory] [WARN] Unexpected end of compressed LZ4 block data at chunk header for block #" + std::to_string(i) + " in " + entryName + ".\n");
					currentBlockData.clear();
					break;
				}

				int32_t rawChunkSize;
				std::memcpy(&rawChunkSize, compressedData + readChunkPos, sizeof(int32_t));
				readChunkPos += sizeof(int32_t);

				int32_t currentChunkDataSize = rawChunkSize & 0x7FFFFFFF;

				if (currentChunkDataSize == 0 || readChunkPos + static_cast<size_t>(currentChunkDataSize) > compressedBlockActualSize) {
					LogError("[ConvertToDDSInMemory] [WARN] Invalid chunk data size/bounds for chunk at LZ4 block #" + std::to_string(i) + " in " + entryName + ", offset " + std::to_string(readChunkPos - 4) + ". Skipping chunk.");
					currentBlockData.clear();
					break;
				}
				...
				if (bytesProcessed <= 0) {
					LogError("[ConvertToDDSInMemory] [ERROR] LZ4 decompress error at block #" + std::to_string(i) + " in " + entryName + ", chunk offset " + std::to_string(readChunkPos - 4) + ". Error code: " + std::to_string(bytesProcessed) + ". Aborting chunk processing.\n");
					currentBlockData.clear();
					break;
				}

				outPos += bytesProcessed;
				readChunkPos += static_cast<size_t>(currentChunkDataSize);
			}

			if (!currentBlockData.empty() && static_cast<uint32_t>(outPos) != decompressedTotalSize) {
				LogError("[ConvertToDDSInMemory] [WARN] Decompressed size mismatch for LZ4 block #" + std::to_string(i)
					+ " in " + entryName + " (expected " + std::to_string(decompressedTotalSize) + ", got " + std::to_string(outPos) + "). This might indicate a parsing issue.");
			}
	...
	ddsResult.reserve(ddsHeader.size() + ddsHeaderDx10.size() + (headerFixed ? firstMipMapSize : decodedData.size()));

	ddsResult.insert(ddsResult.end(), ddsHeader.begin(), ddsHeader.end());
	if (!ddsHeaderDx10.empty()) {
		ddsResult.insert(ddsResult.end(), ddsHeaderDx10.begin(), ddsHeaderDx10.end());
	}

	if (headerFixed) {
		ddsResult.insert(ddsResult.end(), decodedData.begin(), decodedData.begin() + firstMipMapSize);
	}
	else {
		ddsResult.insert(ddsResult.end(), decodedData.begin(), decodedData.end());
	}
